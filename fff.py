...
async def start(message: types.Message):
    await message.answer("Добро пожаловать в бот для предложений!", reply_markup=Keyboards.main())
...
async def start_suggestion(message: types.Message, state: FSMContext):
    ...
    await message.answer("Выберите категорию:", reply_markup=Keyboards.categories())
...
async def confirm(message: types.Message, state: FSMContext):
    ...
    for admin_id in Config.ADMIN_IDS:
        try:
            await bot.send_message(admin_id, f"📨 Новое предложение ({suggestion['category']}) от @{suggestion['author']['username']}")
    ...
    await state.clear()
    await message.answer("✅ Ваше предложение успешно отправлено!", reply_markup=Keyboards.main())

@dp.message(SuggestionStates.COLLECTING_CONTENT, F.text == "➕ Добавить")
async def add_more(message: types.Message, state: FSMContext):
    data = await state.get_data()
    await message.answer(f"Текущее количество элементов: {data.get('count', 0)}")
...
async def collect_item(message: types.Message, state: FSMContext):
    ...
    if count >= Config.MAX_ITEMS:
        return await message.answer("Достигнут лимит элементов. Нажмите ✅ Подтвердить")
    ...
    if item:
        ...
        await message.answer(f"Элемент {count}/{Config.MAX_ITEMS} добавлен!", reply_markup=Keyboards.confirm())
    else:
        await message.answer("Пожалуйста, отправьте допустимый контент.", reply_markup=Keyboards.confirm())
...
async def my_suggestions(message: types.Message):
    ...
    if not suggestions:
        return await message.answer("У вас пока нет отправленных предложений")
...
async def start_web_app():
    app = web.Application()
    app.router.add_get("/", handle_404)
    app.router.add_get("/{tail:.*}", handle_404)  # все остальные пути тоже 404
...
async def main():
    try:
        ...
        logger.exception("Ошибка в одной из задач")
